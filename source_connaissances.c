...
#include <locale.h>
#include "corpus.h"
#include <curl/curl.h> //utilisation de la lib curl pour r�cup�rer plus facilement le contenu avec une pr�sentation correcte

/****************************************************************************************************/
/********fonction recuperant avec libcurl le contenu de sites stock�s dans un fichier (url)*********/
/***************************************************************************************************/
  ...
  if (fichier_lien != NULL)
  {
    ...
    while (fgets(ligne, sizeof ligne, fichier_lien) != NULL)
    {
      ...
      else
      {
        ...
        FILE *fichier_final = fopen(fichier_final_nom, "wb");                     //cr�ation du fichier final
        if (fichier_final == 0)
          printf("erreur\n");
        curl_easy_setopt(session, CURLOPT_WRITEDATA, fichier_final); //ligne pas fichier_final ?
        curl_easy_setopt(session, CURLOPT_WRITEFUNCTION, fwrite);
...
int conserver_lien(char *nom_fichier, char *nom_fichier_2, int nombre_source)
{
  ...
  long taille_fichier;
  char *buffer, *taille_lien, *debut, *fin, *balise_debut, *balise_fin, *ptrdebut, lien[255];
  int nbWikipedia = 0, nbSuperProf = 0, nbMymusicteacher = 0;
  int compt = 0;
  ...
  balise_debut = "<a href=\"/url?q="; //meilleur recherche mais retirer 1000
  balise_fin = "&amp";
  ...
  while (debut) // boucle pour traiter tout les liens de la page
  {
    ...
    ptrdebut = debut;                         // Pointeur pour qu'on garde le début après vérification du lien
    ...
    if (compt < nombre_source)
    {
      ...
      lien[i] = '\0';
      debut = ptrdebut;
      taille_lien = (char *)(fin - debut) - 1;
      if (
          (strstr(lien, ".pdf") == 0) &&
          (strstr(lien, "cdiscount") == 0) &&
          (strstr(lien, "support") == 0) &&
          (strstr(lien, "accounts.google") == 0) &&
          (strstr(lien, "youtube") == 0) &&
          (strstr(lien, "cultura") == 0) &&
          (strstr(lien, "amazon") == 0) &&
          (strstr(lien, "fnac") == 0) &&
          (strstr(lien, "leboncoin") == 0) &&
          (strstr(lien, "woodbrass") == 0) && 
          (strstr(lien, "cdiscount") == 0) &&
          (strstr(lien, "wikipedia") == 0 || nbWikipedia < 1) &&
          (strstr(lien, "mymusicteacher") == 0 || nbMymusicteacher < 1) &&
          (strstr(lien, "superprof") == 0 || nbSuperProf < 1))
      {
        if (strstr(lien, "wikipedia")) nbWikipedia ++;
        if (strstr(lien, "superprof")) nbSuperProf ++;
        if (strstr(lien, "mymusicteacher")) nbMymusicteacher ++;
        while (taille_lien != 0)
        {
          taille_lien--;
          debut++;
...
int faire_corpus(char *fichier_dico, char *nom_rep_corpus)
{
  ...
  if (fichier_lien != NULL)
  {
    while (fgets(ligne, sizeof ligne, fichier_lien))
    {
      ...
      printf("\nMot du lexique des instruments de musique: %s \nRecuperation de %d sources pour ce mots\n\n", ligne, nombre_source);
      ...
      aux = concatene("https://www.google.fr/search?q=histoire+sur+l'instrument+", ligne);
